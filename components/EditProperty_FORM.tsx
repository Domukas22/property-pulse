...
export default function EditProperty_FORM() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const FETCH_propertyData = async () => {
      try {
        const property_DATA = await FETCH_singleProperty(id);

        SET_fields(property_DATA);
      } catch (error) {
        console.error(error);
  ...
  const HANLDE_submit = async (e: any) => {
    e.preventDefault();
    try {
      const form_DATA = new FormData(e.target);
      const res = await fetch(`/api/properties/${id}`, { method: "PUT", body: form_DATA });
